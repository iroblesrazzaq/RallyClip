...
## Session 10b - 2026-07-04 (same session, second feature)

System-webview shell shipped (feat/pywebview-shell, PR #27 stacked on #26): QtWebEngine/PySide6 replaced by pywebview (WKWebView/WebView2) over the unchanged Flask backend. Key insight: the Qt-Multimedia native player + QWebChannel bridge existed only because QtWebEngine's Chromium has no H.264/HEVC; the system webview plays both natively, and the frontend already fell back to its HTML5 player when the bridge was absent — so the shell swap needed zero frontend changes and deleted 1600 lines. Bundle: 765MB -> 266MB .app, 284MB -> 113MB zipped (v0.1.0 dmg 558MB). Verified: WKWebView driven probe (play/seek/upload), bundled --cli golden byte-equal, headless upload->library job exact golden segments, fast+e2e suites green. PyAV is now the only bundled FFmpeg; remaining dual-ffmpeg work is a videoio-less OpenCV build (cv2's 75MB ffmpeg is direct-linked, needs a custom wheel).
