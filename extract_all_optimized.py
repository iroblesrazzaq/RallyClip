#!/usr/bin/env python3
"""
extract_all_optimized.py - Batch extract pose data from all videos using the optimized extractor

This script finds all video files in the raw_videos directory and runs pose extraction
on each one using the pose_extractor_optimized.py script.
...
Examples:
    python extract_all_optimized.py                                    # Default: 0s to 60s, 15 FPS, 0.05 conf, small model, batch 8
    python extract_all_optimized.py 0 30 10 0.03 m 16                  # 0s to 30s, 10 FPS, 0.03 conf, medium model, batch 16
    python extract_all_optimized.py 30 60 15 0.05 l 4                  # 30s to 90s, 15 FPS, 0.05 conf, large model, batch 4
"""
...
def run_extraction_command(video_path, start_time, duration, target_fps, confidence_threshold, model_size, batch_size):
    """
    Run the optimized pose extraction command.
    ...
    """
    cmd = ["python", "pose_extractor_optimized.py", str(start_time), str(duration), str(target_fps), str(confidence_threshold), video_path, model_size, str(batch_size)]
...
def main():
    ...
    print("🎯 Batch Extracting Pose Data from All Videos (Optimized Version)")
    print("=" * 60)
    ...
    for i, video_path in enumerate(video_files, 1):
        ...
        pose_data_path = f"pose_data/unfiltered/{subdir_name}/{base_name}_posedata_{start_time}s_to_{start_time + duration}s_yolo{model_size}_optimized.npz"
        ...
        # Run extraction
        if run_extraction_command(video_path, start_time, duration, target_fps, confidence_threshold, model_size, batch_size):
            successful += 1
        else:
            failed += 1
